<template>
  <div id="app" class="container" :style="{'background-image':'url('+bgImg+')','min-width':'720px','min-height': appHeight+'px'}">
    ...
    <ul ref="labels" class="labels">
      <ul v-for="row in rowCount" :key="row">
        <li v-for="(item,index) in labels.slice((rowCount-row)*colCount,(rowCount-row+1)*colCount)" @click="checkLabel(index,rowCount-row,item)" :key="index" :style="{'background':'radial-gradient(#ccc, '+ (item.color?item.color:colors[(index+(rowCount-row)*colCount)%11])+')'}" >{{item.title}}</li>
      </ul>
    </ul>
    ...
    <el-drawer
      ...
      :show-close="false"
      @close="(option.labels.model==3)&&(option.labels.model=0)">
      <div class="labelform">
        <label>标题</label>
        <el-input v-model="labelForm.title"></el-input><br>
        <label>网址</label>
        <el-input v-model="labelForm.url"></el-input><br>
        <label>颜色</label>
        <el-color-picker
      ...
      <div class="labelfooter">
        <el-button  @click="labelDialog=false">取消</el-button>
        <el-button type="primary" @click="updateLabel">提交</el-button>
      </div>
    </el-drawer>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      labelDialog:false,
      colors:['#ff4500','#ff8c00','#ffd700','#90ee90','#00ced1','#1e90ff','#c71585','#c23531','#2f4554', '#61a0a8', '#d48265', '#91c7ae','#ca8622','#6e7074']
    };
  },
  mounted(){
    ...
    window.onresize = () => {
      var _rect=this.$refs["labels"].getClientRects()[0]
      if(_rect.width<this.labelwidth||_rect.width>this.labelwidth+70){
        this.colCount=parseInt(_rect.width/70)
        this.rowCount=Math.ceil(this.labels.length/this.colCount)
        this.labelwidth=70*this.colCount+40
        this.appHeight=240+this.rowCount*70
      }
    };
  },
  methods:{
    labelChange(rect){
      try{
        ...
        if(labels&&labels.length>0){
          this.colCount=parseInt(rect.width/70)
          this.labels=labels
          this.rowCount=Math.ceil(labels.length/this.colCount)
          this.labelwidth=70*this.colCount+40
          this.appHeight=240+this.rowCount*70
        }
      }catch(e){
    ...
    updateLabel(){
      if(this.option.labels.model==2){
        var index=this.labelForm.index
        delete this.labelForm.index
        this.labels[index]=this.labelForm
      }else{
        this.labels.push(this.labelForm)
        var rect=this.$refs["labels"].getClientRects()[0]
        this.colCount=parseInt(rect.width/70)
        this.rowCount=Math.ceil(this.labels.length/this.colCount)
        this.labelwidth=70*this.colCount+40
      ...
      this.$refs.labelDialog.closeDrawer()
    },
    changeLabel(value){
    ...
    checkLabel(x,y,item){
      ...
      var index=x+y*this.colCount
      if(this.option.labels.model==1){
        this.labels.splice(index,1)
      }else if(this.option.labels.model==2){
        this.labelForm=item
        this.labelForm.index=index
        this.labelDialog=true
      }
      localStorage.setItem('labels',JSON.stringify(this.labels))
    },
